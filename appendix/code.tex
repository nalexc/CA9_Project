...
&= \ddot{R} \cdot \vec{\hat{x}} + 2w\dot{R} \cdot \vec{\hat{y}} - w^2R \cdot \vec{\hat{x}} \\
...
\right.
\end{equation}
The operating point is the position ($x*,y*$) of the satellite 2 in the frame of satellite. $x*$ and $y*$ can be computed thanks to the \figref{fig:operating_pt}.
\begin{figure}[H]
	\centering
...
x* &= -d*sin(\beta_2) \\
&= -d*sin(\frac{\alpha}{2}) \\
&= -2R*sin(\frac{\alpha}{2})^2 \\
y* &= d*cos(\frac{\alpha}{2}) \\
&= 2R*sin(\frac{\alpha}{2})*cos(\frac{\alpha}{2}) \\
&= R*sin(\alpha)
\end{flalign*}
...
\begin{aligned}
	...
	&2(x + x*)\frac{\mu}{R^3} + \frac{u_1}{m} ||\dot{\vec{p_1}}|| \dot{R} - \frac{u_2}{m} ||\dot{\vec{p_2}}||(\dot{R} + \dot{x} - (y + y*)w) + \frac{\Delta F_{dist,x}}{m}\\
