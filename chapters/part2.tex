...
%
\begin{flalign}
	\dot V = s^{T}[-\underline I_{s}^{-1}({\underline S(\vec \omega)\underline I_{s}\vec \omega+\underline S(\vec \omega)\vec h_{mw}+\vec N_{mw}-\vec N_{dis}}-I_{s}\dot{\bar{\omega}})]+I_{s}F\vec{\dot{\tilde{q}}} + u 
	\label{eq:lyap4}
\end{flalign}
%
...
%
\begin{flalign}
	u = {\underline S(\vec \omega)\underline I_{s}\vec \omega+\underline S(\vec \omega)\vec h_{mw}+\vec N_{mw}-\vec N_{dis}}-I_{3\times3}\dot{\bar{\omega}}-I_{s}F\vec{\dot{\tilde{q}}} -I_{s}{\lambda sign(s)}
	\label{eq:controllaw}
	\end{flalign}
%
...
%
\begin{flalign}
	\dot V = -s^{T}\underline I_{s}^{-1}(-I_{s}\dot{\bar{\omega}}-I_{s}{\lambda sign(s)}) 
	\label{eq:lyap5}
\end{flalign}
%
using the notation $\vec{\bar{\omega}} = \vec{\omega}  -\vec{\tilde{\omega}} $ and re-arranging it is obtained
%
\begin{flalign}
	\dot V = -s^{T}(\dot{\tilde{\omega}}+{\lambda sign(s)}) 
	\label{eq:lyap6}
\end{flalign}
%
Choosing $\lambda > {{\dot{\tilde{\omega}}}_{max}} $ the condition $\dot {V} <0 $ is satisfied.
