...
The dynamic and kinematic equations are put together, which will give the nonlinear equations of motion for the satellite.
\begin{flalign}
\begin{bmatrix}
	\vec{\dot q(t)} \\
	\dot \omega{(t)}
...
\begin{bmatrix}
	...
	{-I_{s}^{-1}S(\omega)I_{s}\omega(t)-I_{s}^{-1}S(\omega)h_{tot}+I_{s}^{-1}N_{c}(t)+I_{s}^{-1}N_{dis}(t)}
\end{bmatrix} 
	\label{eq:le}
\end{flalign}
where,\\
  $\vec{\dot q(t)} = [q_1 \ q_2 \ q_3 \ q_4]^T$ \\
  $\dot \omega{(t)} = [ \omega_1 \ \omega_2 \ \omega_3]^T$ \\
  $\underline{\Omega}(\omega)$ is the $4\times4$ skew symmetric matrix \\
  $I_{s}$ is the inertia matrix \\
  $\underline{S}(\omega)$ is the $3\times3$ skew symmetric matrix \\
  $N_{dis}(t)$ is the disturbance torque \\
  $N_{c}(t)$ is the control torque and is equal with $N_{mt}-N_{mw}$, where $N_{mt}$ is the torque from magnetorques and $N_{mw}$ is the torque from momentum wheels 
\subsection{Linearized equation of motion} \label{subsec:lem} 
The kinematic and dynamic equations are linearized for the purpose of designing a linear controller. Thus, the linearized equation of motion for the satellite are given by: 
...
\end{flalign}
where: \\
$\vec{ {\tilde{q}}}$ is the quaternion wich represents a change in attitude \\
$\bar{\omega}$ is the operating point \\
$\tilde{\omega}$ represents the changes of angular velocity around 
