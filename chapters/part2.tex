...
where $[\omega_{1} \ \omega_{2} \ \omega_{3} ]^{T}$ is the angular velocity as seen in the satellite frame.
 Note that the \eqref{eq:lele} has slightly been modified such that the skew symmetric matrix represents the quaternion. According to, for the sliding variable only the vector part of the quaternion is used. \Eqref{eq:quater} can be further modified according as \cite{ref}:
 %
 \begin{flalign}
 	\vec{\dot{\tilde{q_{e}}}} 
 	=
