...
%
\begin{flalign}
	q(t+\Delta{t}) = {q (\Delta {t}) \otimes q(t)}
...
\begin{flalign}
	q(t+\Delta{t})
	= 
	\left\{\cos\frac{\Delta\Phi}{2} I_{(4\times4)}+\sin\frac{\Delta\Phi}{2}
	\begin{bmatrix}
		0 &e_{z}&-e_{y}&e_{x} \\
		-e_{z}&0&e_{x}&e_{y}  \\ 
		e_{y}&-e_{x}&0&e_{z} \\
...
%
\begin{flalign}
	{\frac{d}{dt}(I_{s}{\omega})+\dot{h}_{(mw)}} ={N_{mw}-\omega}     {\times  ({I_{s}}{\omega} +{h_{tot}})}
...
%
\begin{flalign}
	{\dot{\omega}}={-I_{s}^{-1}S(\omega)I_{s}\omega-I_{s}^{-1}S(\omega)h_{mw}-I_{s}^{-1}\dot{h}_{(mw)}-I_{s}^{-1}N_{mw}}
