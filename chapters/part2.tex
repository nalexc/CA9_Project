...
\end{flalign}
%
where $q_{r}$ represents the quaternion reference and $q$ the measured quaternion. The rate of change of quaternion from \Eqref{eq:le} can be  written as 
%
\begin{flalign}
	\vec{\dot{\tilde{q_{e}}}} 
	=
...
where $[\omega_{1} \ \omega_{2} \ \omega_{3} ]^{T}$ is the angular velocity as seen in the satellite frame and $[q_{1e} ..q_{4e}]$ is the quaternion error.
 For the representation of the satellite motion on the sliding variable only the vector part of the quaternion is used. Since only the vector part is used \Eqref{eq:le} can be written:
 %
 \begin{flalign}
 	\vec{\dot{\tilde{q_{e}}}} 
 	=
 ...
 \begin{flalign}
 	s = F\vec{\tilde{q_{e}}} +\vec{\tilde{\omega_{e}}}= 0
 	\label{eq:slidingvar}
 \end{flalign}
where $F = 0.005diag[1 1 1]$ is positive definite and the tilde values represent the quaternion error and the angular velocity error respectively. The angular velocity error can be written as $\vec{\tilde{\omega_{e}}} = \vec{\omega_{cur}}  -\vec{\omega_{obj}} $ Assuming a lyapunov candidate function as
\begin{flalign}
	V = \frac{1}{2}s^{T}s
	\label{eq:lyap}
\end{flalign}
