...
where $\vec{q_{\bar{\omega}}} = \bar{\omega_{1}}*i +  \bar{\omega_{2}}*j + \bar{\omega_{1}}*k + 0$ and $\vec{q_{\tilde{\omega}}} = \tilde{\omega_{1}}*i +  \tilde{\omega_{2}}*j + \tilde{\omega_{1}}*k + 0$. By choosing the sliding manifold variable to be equal:  
 \begin{flalign}
 	...
 	\label{eq:slidingvar}
 \end{flalign}
where $F = a* diag[1 \ 1  \ 1]$ is positive definite. The angular velocity error can be written as $\vec{\tilde{\omega}} = \vec{\omega}  -\vec{\bar{\omega}} $. Therefore, on the manifold, $\vec{ \tilde{\omega} } = -a*\vec{ \tilde{q} }$ and thus the time derivatime of the real part of the error quaternion is computed from the equation \ref{eq:time_deri}:
\begin{flalign}
