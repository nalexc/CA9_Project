...
\end{minipage}
\end{table}
It can be seen that, in the simulation of the state feedback control, the error increases sometimes due to the fact that the linear controller does not take into account the non-linearities of the system. However, in the simulation of sliding mode control, the error quaternion converges as expected. Furthermore, the simulation of the torque generated from the actuator with the linear control method and the non-linear are shown in \figref{fig:torque_state_feedback.eps} and \figref{fig:torque_sliding_mode.eps} respectively. In both cases is illustrated the torque when the drag force alters at the maximum at 500 seconds. It can be seen that at the beginning there is a difference between the sliding mode control and the linear controller since at the sliding mode controller a higher torque needed to converge the states at the manifold. In conclusion the torque with both controllers is very small and acceptable due to the fact that is smaller than the maximum torque  that can be applied at the reaction wheels.   
%
\begin{table}[H]
	\begin{minipage}[b]{0.49\linewidth}
		\centering
