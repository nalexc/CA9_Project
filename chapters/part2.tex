...
%
\begin{flalign}
	\vec q(t+\Delta{t}) = \vec{q (\Delta {t}) \otimes \vec q(t)}
...
%
\begin{flalign}
	\vec q_{1}(\Delta {t})  = {e_{x}\sin\frac{\Delta\Phi}{2}}
	\label{eq:controllerquaternion1}
\end{flalign}
%
\begin{flalign}
	\vec q_{2}(\Delta {t}) = {e_{y}\sin\frac{\Delta\Phi}{2}}
	\label{eq:controllerquaternion2}
\end{flalign}
%
\begin{flalign}
	\vec q_{3} (\Delta {t})= {e_{z}\sin\frac{\Delta\Phi}{2}}
...
\end{flalign}
%
where $\vec{h_{tot}}$ is the vector of the angular momentum of the wheels $[h_1 \ h_2 \ h_3]^{T}$ in the satellites coordinate system and $\underline I_{s}$ is the inertia matrix of the satellite.
%
Inserting the  \eqref{eq:angularmomentum} into \eqref{eq:eulerequation} we obtain:
%
\begin{flalign}
	{\frac{d}{dt}(\underline I_{s}\vec {\omega})+\vec{\dot{h}_{(mw)}}} =\vec{{N_{mw}}-\vec{\omega}}     {\times  ({\underline I_{s}}{\vec{\omega}} +{\vec{h_{tot}}})}
...
%
\begin{flalign}
	{\vec{\dot{\omega}}}={-\underline I_{s}^{-1}\underline S(\omega)\underline I_{s}\omega-\underline I_{s}^{-1}\underline S(\omega)\vec h_{mw}-\underline I_{s}^{-1}\vec {\dot{h}_{(mw)}}-\underline I_{s}^{-1}\vec {N_{mw}}}
...
%
\begin{flalign}
	{\vec{\dot{\omega}}} ={-\underline I_{s}^{-1}\underline S(\omega)\underline I_{s}\vec \omega-\underline I_{s}^{-1}\underline S(\omega)\vec h_{mw}-\underline I_{s}^{-1}\vec N_{mw}+\underline I_{s}^{-1}\vec N_{dis}}
...
\begin{flalign}
	...
	-S(\bar{\omega}) &	\frac{1}{2} \underline{\vec 1}_{(3\times3)} \\
	 \underline{\vec 0}_{(3\times3)} &	{I_{s}^{-1}S(I_{s}\omega(t))-I_{s}^{-1}S(\omega)I_{s}}
	\end{bmatrix} 
		\begin{bmatrix}
		\vec{  {\tilde{q}}(t) } \\
		{  {\tilde{\omega}}(t) }
...
\begin{flalign}
	\vec N_{rad} = F_{rad} \times \vec R_{CoM}
	\label{eq:tor}
\end{flalign}
where $ F_{rad}$  is the solar radiation  and $\vec R_{CoM}$ is the vector from the centre of mass to the geometric centre of radiation
...
\begin{flalign}
	\vec N_{drag} = \vec r_{s}  \times F_{D}
	\label{eq:drag}
\end{flalign}
where:\\
...
The function $f(q)$ is computed in the pathm between the minimum and maximum drag force:
\begin{flalign}
	q = q_y \otimes q_z \\
\end{flalign}
with
...
The \figref{fig:path_alpha} shows $f(q)$ as a function of $\alpha$:
\begin{figure}[H]
	\centering
...
\begin{flalign}
	 \vec{\tilde{q_{e}}} = \vec{\bar{q}}^{-1} \otimes \vec{q} 
	 =
	\begin{bmatrix}
...
where $[\omega_{1} \ \omega_{2} \ \omega_{3} ]^{T}$ is the angular velocity as seen in the satellite frame.
 Note that the \eqref{eq:lele} has slightly been modified such that the skew symmetric matrix represents the quaternion. According to, for the sliding variable only the vector part of the quaternion is used. \Eqref{eq:quater} can be further modified according as:
 %
 \begin{flalign}
 	\vec{\dot{\tilde{q_{e}}}} 
 	=
 ...
 \begin{flalign}
 	s = F\vec{\tilde{q_{e}}} +\vec{\tilde{\omega}}= 0
 	\label{eq:slidingvar}
 \end{flalign}
