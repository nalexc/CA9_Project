...
The time derivative of $\vec{^s_i q(t)}$ can be computed:
 \begin{flalign}
	\vec{^s_i q(t+\Delta{t})} = \lim_{\Delta t\to 0} \frac{q(t+\Delta t) - q(t)}{\Delta t} = \dfrac{1}{2} \underline \Omega \  \vec{^s_i q(t)}
	\label{eq:asdd}
\end{flalign} 
...
Due to the fact that the above A matrix is affine in $\vec{\bar{\omega}}$, the stability can be only checked on the vertices of the convex polyhedron (cube) that contains all the possibilities of $\vec{\bar{\omega}}$ as represented in the \figref{fig:sta_PID}. \cite{NLCS}
\begin{figure}[H]
	\centering
